...
spk_model = SpkModel("model-spk_old") #
...
async def index2(request):
    content = open(str(ROOT / 'static' / 'index_2.html')).read()
    return web.Response(content_type='text/html', text=content)
#----------
...
#-----------
async def send_signature(request):
    spkembd = None
    ...
    #     f.write(data)
    #full_audio_bytes = b''.join(data)  # this is my table of chunks of audio data        
    with wave.open("myfile.wav", "wb") as audiofile:
        audiofile.setsampwidth(2)
        audiofile.setnchannels(1)
        audiofile.setframerate(44100)
        audiofile.writeframes(data)
    #print(data)
    wf = wave.open('myfile.wav','rb')
    #print(wf)
    while True:
        data_ = wf.readframes(2000)
        if len(data_) == 0:
            break
        if rec.AcceptWaveform(data_):
            res = json.loads(rec.Result())
            print(res)
            if 'spk' in res:
                spkembd = res['spk']
    ...
    # print(res)
    # while True:  #when start is pressed
    #     dataframes_ = bytearray(b'')
    #     max_frames_len = 8000 #max buffer size trial and test
    #     message = data#.planes[0].to_bytes() 
    #     #print(len(str(message).split('\\')))
    #     recv_frames_ = bytearray(message)
    #     #print(len(recv_frames),'recv')
    #     dataframes_ += recv_frames_ 
    #     #print(len(dataframes),'df')
    #     if len(dataframes_) > max_frames_len:
    #         wave_bytes_ = bytes(dataframes_)
    #         response = await self.loop.run_in_executor(pool, process_chunk, self.__recognizer, wave_bytes_)
    #         if 'spk' in response:
    #             print(response)
    #         #print(speaker)
    #         #print('-----')
    return web.Response(text=str(spkembd),content_type="text/html")
    #return web.Response(text='[-0.701766, 0.313454, 0.285104, -1.157942, -1.025656, 0.359487, -0.397543, -0.738973, 1.633481, -0.077619, 0.011297, -0.023481, -0.990958, -0.921590, -0.902737, -0.254562, 0.478593, 0.081016, 2.043904, 0.215783, -0.296242, 2.049364, 1.969494, -0.381765, -1.040035, -0.636572, -0.671292, -1.647781, 0.562115, 0.728833, -0.559134, -1.370062, 0.188702, -1.041054, 0.293849, -2.449665, 1.236091, 1.086694, -0.957984, 0.546023, 0.734656, 1.330279, 0.438040, 0.081506, -0.930533, 1.371469, 0.052700, -0.599288, 0.187568, 0.868085, 0.107612, 0.932629, 0.196172, 1.043293, -0.365125, 0.157180, 0.250023, -0.250561, -0.656860, -1.249357, 0.954533, 0.354545, 0.005050, -1.438345, -0.137508, -0.992737, -0.565105, 0.281953, -1.195023, 2.748891, -0.586424, -2.366060, -0.605496, -0.659102, 0.303107, 0.115082, -1.137468, -0.366137, -0.188624, -1.305033, 0.387655, -1.230271, -0.223709, -0.165224, -0.592402, -0.331704, 0.071005, 1.486784, 0.599618, -0.233855, 0.503320, -0.620836, -0.224790, 0.052773, -0.929046, 0.081823, -0.366624, -1.166075, 0.810104, 2.258941, 0.908314, -0.301767, -0.180385, -1.584045, -0.037256, -1.478272, 0.285490, -1.095387, -0.696070, -0.104534, -1.608816, 0.968370, 1.192020, -1.002699, 2.961337, 0.216760, -0.145203, 1.073298, -0.002436, -0.253612, 0.700424, 1.986816, -1.314074, 1.693625, 0.702341, 1.354820, -2.112917, 0.155190]',content_type="text/html")
#-----------
...
if __name__ == '__main__':
    ...
    app.router.add_get('/sendfile', index2)
    app.router.add_post('/sendfile', send_signature)
